ALTER TABLE public.teachers ADD COLUMN gender TEXT NULL;

DROP FUNCTION IF EXISTS public.get_public_teachers();

CREATE OR REPLACE FUNCTION public.get_public_teachers()
RETURNS TABLE(id uuid, first_name text, last_name text, gender text, instruments text[])
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT t.id, t.first_name, t.last_name, t.gender,
  ...
  GROUP BY t.id, t.first_name, t.last_name, t.gender
