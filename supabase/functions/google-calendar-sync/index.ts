...
function itemToEvent(item: Json): Json {
  ...
  if (item.start_time) {
    ...
    let endDate = String(item.end_date);

    // Google rejects a timed event whose end is not after its start. Old rows may
    // contain reversed same-day times, so keep sync resilient while the UI blocks
    // any new invalid values.
    if (endDate === String(item.start_date) && endTime <= startTime) {
