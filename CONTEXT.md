...
## אבטחה — מה תוקן (08/06/2026)

- מורים **אין** גישה ל-`student_payments` (הוסר — invoice URLs ו-iCount IDs לא חשופים למורים)
- הופעלה הגנת סיסמאות דלופות (HIBP) ב-Supabase Auth
- נקבע `search_path` קבוע על פונקציות תור המייל
- **מכוון (לא באג)**: מידע יצירת קשר של מחנכים ב-school music חשוף למורים/רכזים — לצורך תיאום שיעורים
...
## עבודה אחרונה (יולי 2026)

### טופס הרשמה ציבורי — כותרת ✅
- `PublicRegistration.tsx` — כותרת קבועה: "אולפן המוסיקה "חוף הכרמל" — רישום ומידע" + שנה דינמית
- שנה נבנית מ-`activeYear.name` (עברית) + גרגוריאני מ-`start_date`/`end_date`
- `document.title` מתעדכן דינמית
- `index.html` — og:title ו-og:image עודכנו (לוגו במקום סקריין כניסה)

### שליחת הודעה להורה — מומש ✅ (Lovable)
**קובץ:** `src/components/admin/SendTeacherAssignmentMessage.tsx`

**מה זה:** Dialog שנפתח מכרטיס תלמיד. מציג הודעה מוכנה לעריכה, שולח בוואטסאפ או מייל.

**נתונים:**
- `enrollments` join `teachers(first_name, last_name, phone)`, `instruments(name)`, `schools(name)`, `lesson_duration_minutes`
- `student_payments` where `payment_status = 'pending'` → `enrollment_breakdown.lines` (מערך `{description, amount}`) + `payment_link_url` + `amount`
- `student.parent_phone` ← לוואטסאפ, `student.parent_email` ← למייל

**מבנה ההודעה:**
- שם הורה + שם תלמיד
- לכל enrollment: כלי, שם מורה, טלפון מורה + קישור WA, שלוחה, משך שיעור
- פירוט תשלום מ-`enrollment_breakdown.lines` + סה"כ + אפשרות 10 תשלומים
- קישור לתשלום (`payment_link_url`)
- הערה עריכה: לפני ספטמבר → "ישתחילו בספטמבר", אחרי ספטמבר → "בהקדם האפשרי"
- פרטי יצירת קשר: musichof@gmail.com / 04-6299711 / WA קורין (972547467498)

**שליחה:**
- וואטסאפ: `https://wa.me/972[parent_phone]?text=[encoded]`
- מייל: Edge Function `send-transactional-email`, `templateName: "plain-text"`

**שליחה המונית — טרם מומש:** מתוכנן בדף `AdminStudents` — checkbox per student + שליחה אחד-אחד לוואטסאפ. לסנן: תלמידים עם enrollment פעיל + pending payment עם payment_link_url.

---

### שינויים נוספים (Lovable, יולי 2026)

**הנחות — ארכיטקטורה חדשה:**
- `src/lib/discounts.ts` + `src/lib/__tests__/discounts.test.ts`
- טבלת `discount_types` בDB: הנחות דינמיות לפי שנת לימוד עם `applies_to: "all" | "cheapest_enrollment"`
- `"cheapest_enrollment"` = הנחה על כל ה-enrollments חוץ מהיקר ביותר (מחליף לוגיקת "כלי שני" ישנה)
- `computeStandardDiscounts()` — חישוב הנחות עם breakdown מפורט

**קומפוננטים חדשים בכרטיס תלמיד:**
- `RegistrationApprovalSection.tsx` — מציג הרשמה אחרונה (שנה, תאריך) + כפתור צפייה
- `StudentEnsemblesSection.tsx` — מציג הרכבים של התלמיד

**מייל:**
- תבנית מייל מנהלים (`יצרתי תבנית מייל מנהלים`) — פרטים לא ברורים, לבדוק

**UI/UX:**
- שיפורי mobile, סינון סוג רישום, שדה רישום בעריכת תלמיד
- תוקן ברירת מחדל משך שיעור
- שיפורי כרטיסי תלמיד ומורה
