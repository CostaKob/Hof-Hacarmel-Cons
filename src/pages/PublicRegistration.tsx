...
const PublicRegistration = () => {
  ...
  const { data: activeYear, isLoading: yearLoading } = useQuery({
    queryKey: ["public-active-year", urlYearParam, urlYearId],
    queryFn: async () => {
      // 1. Try by year name (e.g. "2025-2026")
      if (urlYearParam) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // (sync-registration-to-sheets is restricted to service-role callers).
      try {
    ...
    <div className="min-h-screen bg-muted/30 py-6 px-4" dir="rtl">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          {studyDetailsIdx < 0 && <SpecialCoursesCard />}
