...
const SchoolMusicRegisterSuccess = () => {
  ...
  const status = params.get("status"); // "ok" or "cancel"
  const [pollCount, setPollCount] = useState(0);

  const { data: payment, isLoading } = useQuery({
    queryKey: ["sm-payment-after-pay", paymentId, pollCount],
    enabled: !!paymentId,
    queryFn: async () => {
    ...
    if (status === "cancel") {
      return (
        <>
          ...
          <p className="text-muted-foreground">ההרשמה נשמרה. תוכלו להשלים את התשלום בהמשך באמצעות קישור שנשלח אליכם.</p>
        </>
      );
    }
...
export default SchoolMusicRegisterSuccess;
